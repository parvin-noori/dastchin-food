...
export default function CartButton({ products }: { products: ProductItem[] }) {
  ...
  return (
    <>
      ...
      <div
        className={`${collapsed ? "translate-x-0" : "-translate-x-full"} h-full divide-y-2 divide-gray-200  duration-500 transition-all md:w-xs w-full md:rounded-r-2xl bg-white fixed end-0 top-0 z-20`}
      >
        <button
          onClick={closeCollapsed}
          className="absolute cursor-pointer start-6 top-7 hover:bg-gray-100 rounded-full transiton-color duration-300 p-1.5 border-0"
        >
          <MdClose size={25} />
        </button>
